...
상속관계가 큰 catch는 가장 아래로 보낸다, 즉 최상의 예외 Exception 은 맨 마지막에 와야 한다.
...
- 예외의 최고 조상인 Exception을 처리하는 catch블럭은 모든 종류의 예외를 처리할 수 있다(**반드시 마지막 cathc블럭이어야 한다.**)
...
이렇게 작성할 경우 캐치문 (2), (3)은 에러가 발생하게 된다. 따라서
--->
try {
	tot += arr1[i] / arr2[i];
...
* toString() : System.out.println(); 객체를 담게되면 자동으로 toString() 함수를 실행하게 된다!
