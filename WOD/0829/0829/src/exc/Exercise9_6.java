...
public class Exercise9_6 {
	public static String fillZero(String src, int length) {
		...
		if(src.length() > length) return src.substring(0, length); //3
		char[] carr = new char[length]; //4
		for(int i = 0; i<carr.length;i++) carr[i] ='0'; //5
		System.arraycopy(src.toCharArray(), 0, carr, length-src.length(),src.length()); //6		
